 ...
 */
export function registerIpcHandlers(adbManager: AdbManager, deviceWindows: Map<string, BrowserWindow>): void {
  ...
  ipcMain.handle('device:start-mirror', async (event, serial: string, options?: {
    bitrate?: number;
    maxSize?: number;
    maxFps?: number;
    ...
    if (isWireless) {
        try {
            ...
            if (match) {
                ...
                console.log(`[IPC] Wireless detected. Calculated 50% size: ${defaultMaxSize} (from ${width}x${height})`);
            } else {
                 defaultMaxSize = 600; // Fallback
            }
        } catch (e) {
            console.warn('[IPC] Failed to get device size for wireless default, using 600px', e);
            defaultMaxSize = 600;
        }
    }
    ...
    console.log(`[IPC] Starting mirror for ${serial} (Wireless: ${isWireless}, Bitrate: ${finalOptions.bitrate})`);
  ...
  ipcMain.handle('device:stop-mirror', async (event, serial: string): Promise<void> => {
    // 1. Find and close the device window if it exists
    // This will trigger the 'closed' event in main/index.ts, which calls disconnectDevice and DeviceServer.stop
    ...
    // 2. Fallback: If window doesn't exist, handle cleanup manually
    const connection = connections.get(serial);
    if (connection) {
      connection.disconnect();
      connections.delete(serial);
  ...
  ipcMain.handle('device:connect', async (event, serial: string, port: number): Promise<boolean> => {
    try {
      // Create connection
  ...
  // Send data to device
  ipcMain.handle('device:send', async (event, serial: string, data: Uint8Array): Promise<boolean> => {
    const connection = connections.get(serial);
    if (!connection) {
  ...
  // Get device info
  ipcMain.handle('device:get-info', async (event, serial: string): Promise<DeviceInfo | null> => {
  ...
  // Set manual ADB path
  ipcMain.handle('adb:set-path', async (event, newPath: string): Promise<boolean> => {
  ...
  ipcMain.handle('adb:check-status', async (event, pathToCheck?: string): Promise<boolean> => {
    if (pathToCheck) {
  ...
  ipcMain.handle('adb:shell', async (event, serial: string, command: string): Promise<string> => {
    return adbManager.shell(serial, command);
  });

  // Device window actions
  ipcMain.on('device:nav-button', async (event, serial: string, button: number) => {
    const win = BrowserWindow.fromWebContents(event.sender);
    if (win) {
  ...
  // Screen control
  ipcMain.handle('device:screen-power', async (event, serial: string, on: boolean): Promise<void> => {
    const keycode = on ? 224 : 223;
    await adbManager.shell(serial, `input keyevent ${keycode}`);
  });

  // Rotate screen
  ipcMain.handle('device:rotate', async (event, serial: string): Promise<void> => {
    const current = await adbManager.shell(serial, 'settings get system user_rotation');
    const rotation = ((parseInt(current.trim()) || 0) + 1) % 4;
    await adbManager.shell(serial, `settings put system user_rotation ${rotation}`);
  });

  // Copy logcat
  ipcMain.handle('device:copy-logcat', async (event, serial: string): Promise<string> => {
  ...
  ipcMain.handle('settings:set', async (event, key: string, value: any): Promise<void> => {
    settingsManager.set(key, value);
  });

  // Resize window based on video dimensions
  ipcMain.handle('device:resize-window', async (event, width: number, height: number, chromeHeight: number = 0, chromeWidth: number = 0): Promise<void> => {
    const win = BrowserWindow.fromWebContents(event.sender);
    if (win) {
      ...
      console.log(`[IPC] Resizing window. Video: ${width}x${height}, Chrome: ${chromeWidth}x${chromeHeight}, Scale: ${scale.toFixed(2)} -> Window: ${resultWidth}x${resultHeight}`);
  ...
  // Get device IP
  ipcMain.handle('device:get-ip', async (event, serial: string): Promise<string | null> => {
    return adbManager.getDeviceIp(serial);
  });

  // Enable wireless (TCPIP)
  ipcMain.handle('device:enable-wireless', async (event, serial: string): Promise<void> => {
    return adbManager.enableTcpIp(serial);
  });

  // Connect wireless
  ipcMain.handle('device:connect-wireless', async (event, ip: string, port: number = 5555): Promise<string> => {
    return adbManager.connect(ip, port);
  });
  
  // Disconnect wireless
  ipcMain.handle('device:disconnect-wireless', async (event, serialOrIp: string): Promise<void> => {
